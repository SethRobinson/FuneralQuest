...
#include <string>
#include <shellapi.h>
CRandom g_random; //global random unit
...
void DumpUnfreed()
{
    AllocList::iterator i;
    DWORD totalSize = 0;
    char buf[1024];	      if(!allocList)
        return;	      for(i = allocList->begin(); i != allocList->end(); i++) {
            sprintf(buf, "%s(%d),\t\tADDRESS %Xh\t%d unfreed\n",
...
void getdir(TCHAR t_final_out[])
{
    ...
    for (int k=_tcslen(path);path[k] != '\\'; k--)
    {
        c_cur = k;
    }
    _tcscpy(dir, _T(""));
    //copy file name
    _tcsncat((TCHAR*)&dir, &path[c_cur], _tcslen(path)-c_cur);
    path[c_cur] = 0; //truncate
    _tcscpy(t_final_out, path);
}
...
TCHAR * GetExeDir()
{
    static TCHAR c_st_dir[MAX_PATH] = _T("");
...
TCHAR * stristr(const TCHAR text[], const TCHAR search[])
{
    ...
    //it does us no good to be pointing to something we're about to free, let's point it at the const input we got
    if (p_result)
    p_result = (TCHAR*)&text[(p_result-p_text)];
...
void strip_beginning_spaces(char *s)
{
    ...
    if (s == h) return; //don't really need to do it
    strcpy(s, h);
}

void strip_ending_spaces(TCHAR *p_string)
{
    if (!p_string || !p_string[0]) return;

    for (int i=_tcslen(p_string)-1; p_string[i] == ' '; i++)
    {
        //found a space
        p_string[i] = 0; //truncate it
    }
}
...
/* 
  ...
  which one to return.	  (1 for left most, 2 for after the first separator and so forth)
  Author:  Seth A. Robinson ('97)
*/
...
{
    return g_random.Random(i_max)+i_min;
...
int random_range(long i_small, long i_large)
{
    ...
    return g_random.Random(i_large-i_small)+i_small;
}
...
//checks a file extension.  not case sensitive.  Send ".mp3" to it. 	 
bool file_extension_is(const TCHAR *st_filename, const TCHAR * st_extension)
{
    ...
    ///first find out if there is already a file extension in it
    TCHAR *p_index = (TCHAR *)_tcschr(st_filename, _T('.'));
