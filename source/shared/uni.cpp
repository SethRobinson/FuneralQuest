...
char * uni::to_st()
{
  ...
  if (us_data == NULL)
  {
      ...
      assert(0);
      set_us( (const unsigned short*)L"NULL");
  }
...
{
#ifdef _UNICODE
    return (TCHAR*)us_data;  //unicode mode, get as unicode
#else
...
uni::uni(int i_size)
{
  ...
  //init unicode member
	...
	this->us_data = (WCHAR*) malloc(2*i_size);
...
//added for vc8, why sin't wchar_t and unsigned short treated the same??!
void uni::set_us(const wchar_t * us_new)
{
   set_us((unsigned short*)us_new);
}
void uni::set_us(const unsigned short *us_new)
{
    ...
    if (!us_data)
    {
        assert(!"Malloc failed in uni.cpp");
