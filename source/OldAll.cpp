...
void getdir(char *dir, char *final)
{
	...
	GetModuleFileName(NULL, path, 255);
	CString str = path;
//	Msg("Get dir first says %s",path);
//	Msg("Cstring is %s",(LPCSTR) str);
	int i=str.ReverseFind(92);
    if (i!=-1)
    str=str.Left(i);
	strcpy(final, (LPCSTR)str);
//	Msg("Get current directory says %s.",final);
 }
...
int random(int i_max)
{
 return (abs(rand()%i_max));
}


//Random number + a number, starts at 0
int random(int i_max, int i_min)
{
 return (abs(rand()%i_max))+i_min;
}

//Random number between a defined range
int random_range(int i_small, int i_large)
{
	return (abs(rand()%i_large-i_small))+i_small;
}